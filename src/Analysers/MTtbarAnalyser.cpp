...
void MTtbarAnalyser::analyseFourJetChi2(const EventPtr event) {
	if (topElectronPlusJetsRefSelection_->passesFullSelectionExceptLastTwoSteps(event)) {
		...
		double efficiencyCorrection = event->isRealData() ? 1. : signalElectron->getEfficiencyCorrection(true, Globals::ElectronScaleFactorSystematic);
	...
	if (topMuonPlusJetsRefSelection_->passesFullSelectionExceptLastTwoSteps(event)) {
		...
		double efficiencyCorrection = event->isRealData() ? 1. : signalMuon->getEfficiencyCorrection(false, Globals::ElectronScaleFactorSystematic);
		METPointer met = event->MET();
