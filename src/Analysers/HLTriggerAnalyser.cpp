...
void HLTriggerAnalyser::analyse(const TopPairEventCandidate& ttbarEvent) {
	...
	for (unsigned int index = 0; index < jets.size(); ++index) {
		if (!jets.at(index)->isWithinDeltaR(0.3, mostIsolatedElectron))
			cleanedJets.push_back(jets.at(index));
	}
	...
	if (cleanedJets.size() > 0) {// exactly one jet
		...
		if (isData) {
			passesPreCondition = ttbarEvent.runnumber() > 165970 && (ttbarEvent.HLT(
					HLTriggers::HLT_Ele27_CaloIdVT_CaloIsoT_TrkIdT_TrkIsoT) || ttbarEvent.HLT(
	...
	if (cleanedJets.size() > 1) {
		...
		if (isData) {
			...
			passesPreCondition = ttbarEvent.HLT(HLTriggers::HLT_Ele25_CaloIdVT_TrkIdT_CentralJet30) && ttbarEvent.HLT(
					HLTriggers::HLT_Ele27_CaloIdVT_CaloIsoT_TrkIdT_TrkIsoT);
			prescale = 1;
			passesTrigger = (ttbarEvent.HLT(HLTriggers::HLT_Ele25_CaloIdVT_TrkIdT_DiCentralJet30) || ttbarEvent.HLT(
					HLTriggers::HLT_Ele25_CaloIdVT_TrkIdT_CentralDiJet30)) && ttbarEvent.HLT(
					HLTriggers::HLT_Ele27_CaloIdVT_CaloIsoT_TrkIdT_TrkIsoT);
		}
		analyseTrigger(passesPreCondition, passesTrigger, histFolder, jet2, prescale);
	}
	...
	if (cleanedJets.size() > 2) {
		...
		if (isData) {
			...
			passesPreCondition = (ttbarEvent.HLT(HLTriggers::HLT_Ele25_CaloIdVT_TrkIdT_DiCentralJet30)
					|| ttbarEvent.HLT(HLTriggers::HLT_Ele25_CaloIdVT_TrkIdT_CentralDiJet30)) && ttbarEvent.HLT(
					HLTriggers::HLT_Ele27_CaloIdVT_CaloIsoT_TrkIdT_TrkIsoT);
			prescale = 1;
			//emulate trigger
			passesTrigger = (ttbarEvent.HLT(HLTriggers::HLT_Ele25_CaloIdVT_TrkIdT_TriCentralJet30) || ttbarEvent.HLT(
					HLTriggers::HLT_Ele25_CaloIdVT_TrkIdT_CentralTriJet30)) && ttbarEvent.HLT(
					HLTriggers::HLT_Ele27_CaloIdVT_CaloIsoT_TrkIdT_TrkIsoT);
		}
		analyseTrigger(passesPreCondition, passesTrigger, histFolder, jet3, prescale);
		...
		if (cleanedJets.size() > 3) {
			...
			if(isData){
				...
				prescale = 1;
				passesPreCondition = (ttbarEvent.HLT(HLTriggers::HLT_Ele25_CaloIdVT_TrkIdT_TriCentralJet30)
						|| ttbarEvent.HLT(HLTriggers::HLT_Ele25_CaloIdVT_TrkIdT_CentralTriJet30)) && ttbarEvent.HLT(
						HLTriggers::HLT_Ele27_CaloIdVT_CaloIsoT_TrkIdT_TrkIsoT);
				//no emulation so far
				passesTrigger = false;
			}
...
//		if ((ttbarEvent.GoodPFIsolatedElectrons().size() > 0 || ttbarEvent.GoodIsolatedElectrons().size() > 0)
...
//				const JetPointer jet1(cleanedJets.front());
//				histFolder = "HLTStudy/HLT_Ele25_CaloIdVT_TrkIdT_CentralJet30";
//				passesPreCondition = true;//no pre-trigger other than the EWK triggers
//				passesTrigger = ttbarEvent.HLT(HLTriggers::HLT_Ele25_CaloIdVT_TrkIdT_CentralJet30);
//				prescale = ttbarEvent.HLTPrescale(HLTriggers::HLT_Ele25_CaloIdVT_TrkIdT_CentralJet30);
//				analyseTrigger(passesPreCondition, passesTrigger, histFolder, jet1, prescale);
//
...
//		}
	...
	void HLTriggerAnalyser::createHistograms() {
		...
		// ele tight ID + 1 jet
		histMan->addH1D_JetBinned("jet_pt_visited", "HLT_Ele25_CaloIdVT_TrkIdT_CentralJet30 jet pt (visited)", 200, 0,
				200);
		histMan->addH1D_JetBinned("jet_eta_visited", "HLT_Ele25_CaloIdVT_TrkIdT_CentralJet30 jet eta (visited)", 80,
				-4, 4);
		histMan->addH1D_JetBinned("jet_phi_visited", "HLT_Ele25_CaloIdVT_TrkIdT_CentralJet30 jet phi (visited)", 80,
				-4, 4);
		...
		// ele tight ID + 2 jet
		histMan->addH1D_JetBinned("jet_pt_visited", "HLT_Ele25_CaloIdVT_TrkIdT_DiCentralJet30 jet pt (visited)", 200,
				0, 200);
		histMan->addH1D_JetBinned("jet_eta_visited", "HLT_Ele25_CaloIdVT_TrkIdT_DiCentralJet30 jet eta (visited)", 80,
				-4, 4);
		histMan->addH1D_JetBinned("jet_phi_visited", "HLT_Ele25_CaloIdVT_TrkIdT_DiCentralJet30 jet phi (visited)", 80,
				-4, 4);

		histMan->addH1D_JetBinned("jet_pt_fired", "HLT_Ele25_CaloIdVT_TrkIdT_DiCentralJet30 jet pt (fired)", 200, 0,
				200);
		histMan->addH1D_JetBinned("jet_eta_fired", "HLT_Ele25_CaloIdVT_TrkIdT_DiCentralJet30 jet eta (fired)", 80, -4,
				4);
		histMan->addH1D_JetBinned("jet_phi_fired", "HLT_Ele25_CaloIdVT_TrkIdT_DiCentralJet30 jet phi (fired)", 80, -4,
				4);

		histMan->setCurrentCollection("HLTStudy/HLT_Ele25_CaloIdVT_TrkIdT_TriCentralJet30");
		// ele tight ID + 3 jet
		histMan->addH1D_JetBinned("jet_pt_visited", "HLT_Ele25_CaloIdVT_TrkIdT_TriCentralJet30 jet pt (visited)", 200,
				0, 200);
		histMan->addH1D_JetBinned("jet_eta_visited", "HLT_Ele25_CaloIdVT_TrkIdT_TriCentralJet30 jet eta (visited)", 80,
				-4, 4);
		histMan->addH1D_JetBinned("jet_phi_visited", "HLT_Ele25_CaloIdVT_TrkIdT_TriCentralJet30 jet phi (visited)", 80,
				-4, 4);

		histMan->addH1D_JetBinned("jet_pt_fired", "HLT_Ele25_CaloIdVT_TrkIdT_TriCentralJet30 jet pt (fired)", 200, 0,
				200);
		histMan->addH1D_JetBinned("jet_eta_fired", "HLT_Ele25_CaloIdVT_TrkIdT_TriCentralJet30 jet eta (fired)", 80, -4,
				4);
		histMan->addH1D_JetBinned("jet_phi_fired", "HLT_Ele25_CaloIdVT_TrkIdT_TriCentralJet30 jet phi (fired)", 80, -4,
				4);

		histMan->setCurrentCollection("HLTStudy/HLT_Ele25_CaloIdVT_TrkIdT_QuadCentralJet30");
		// ele tight ID + 4 jet
		histMan->addH1D_JetBinned("jet_pt_visited", "HLT_Ele25_CaloIdVT_TrkIdT_QuadCentralJet30 jet pt (visited)", 200,
				0, 200);
		histMan->addH1D_JetBinned("jet_eta_visited", "HLT_Ele25_CaloIdVT_TrkIdT_QuadCentralJet30 jet eta (visited)",
				80, -4, 4);
		histMan->addH1D_JetBinned("jet_phi_visited", "HLT_Ele25_CaloIdVT_TrkIdT_QuadCentralJet30 jet phi (visited)",
				80, -4, 4);

		histMan->addH1D_JetBinned("jet_pt_fired", "HLT_Ele25_CaloIdVT_TrkIdT_QuadCentralJet30 jet pt (fired)", 200, 0,
				200);
		histMan->addH1D_JetBinned("jet_eta_fired", "HLT_Ele25_CaloIdVT_TrkIdT_QuadCentralJet30 jet eta (fired)", 80,
				-4, 4);
		histMan->addH1D_JetBinned("jet_phi_fired", "HLT_Ele25_CaloIdVT_TrkIdT_QuadCentralJet30 jet phi (fired)", 80,
				-4, 4);
		...
		histMan->addH1D_JetBinned("jet_pt_fired",
				"HLT_Ele25_CaloIdVT_CaloIsoT_TrkIdT_TrkIsoT_CentralJet30 jet pt (fired)", 200, 0, 200);
