...
MCParticle::MCParticle(double energy, double px, double py, double pz) :
...
bool MCParticle::isNeutrino() const {
	if ((abs(pdgId()) == 12) || (abs(pdgId()) == 14) || (abs(pdgId()) == 16) || (abs(pdgId()) == 18))
		return true;
	else
		return false;
}
