...
def NormalisationAnalysis(bjetbin):
    global metbins, metsystematics_sources, N_Events
    analysisTimer = Timer()
...
def CrossSectionAnalysis(input, bjetbin):
...
def NormalisedCrossSectionAnalysis(input, bjetbin):
    global N_ttbar_by_source
    result = {}
    theoryXsection = 157.5
    sums = {}
...
def printNormalisationResult(result, bjetbin, toFile=True):
    global metbins
    printout = '\n'
    printout += '=' * 60
    printout = '\n'
...
def calculateTotalUncertainty(results):
    ...
    totalPlus = sqrt(totalPlus)
    totalMinus = sqrt(totalMinus)
    totalPlus_err = sqrt(totalPlus_err)
    totalMinus_err = sqrt(totalMinus_err)
...
def printNormalisationResultsForTTJetWithUncertanties(result, bjetbin, toFile=True):
    global metbins
    printout = '\n'
    printout += '=' * 60
    printout = '\n'
...
def printCrossSectionResult(result, bjetbin, toFile=True):    
    global metbins
    printout = '\n'
    printout += '=' * 60
    printout = '\n'
    ...
    for metbin in metbins:
        ...
        for source in result[metbin].keys():
            ...
            text = ' $%(value).2f \pm %(error).2f$  pb' % fitresult + '(%.2f' % (relativeError * 100) + '\%)'
...
def printCrossSectionResultsForTTJetWithUncertanties(result, bjetbin, toFile=True):
    global metbins
    printout = '\n'
    printout += '=' * 60
    printout = '\n'
...
def plotNormalisationResults(results, bjetbin):
    global metbins
    for metbin in metbins:
...
def printNormalisedCrossSectionResultsForTTJetWithUncertanties(result, bjetbin, toFile = True):
    global metbins
    printout = '\n'
    printout += '=' * 60
    printout = '\n'
